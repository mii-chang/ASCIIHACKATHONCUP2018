...
public class CFXEasyEditor : EditorWindow {
	...
	private void CopyModules(ParticleSystem source, ParticleSystem dest) {
		...
		if (b_modules[0]) {
			...
			destEm.rateOverTime = source.emission.rateOverTime;
			destEm.rateOverDistance = source.emission.rateOverDistance;
	...
	private void ScaleParticleValues(ParticleSystem ps, GameObject parent) {
		...
		if (main.startSizeMultiplier > 0.01f) main.startSizeMultiplier *= ScalingValue;
